...
def get_bumble(files_found, report_folder, seeker, wrap_text):
    ...
    if usageentries > 0:
        ...
        tsvname = 'Bumble - Messages'
        tsv(report_folder, data_headers, data_list, tsvname)

        tlactivity = 'Bumble - Messages'
    ...
    if usageentries > 0:
        for row in all_rows1:
            ...
            if row[1] == 'userId':
                ...
                data_list_account.extend(
                    (
                        ('Timestamp', str(bumble_timestamp)),
                        ('Last Latitude', bumble_lastlat),
                        ('Last Longitude', bumble_lastlong),
                    )
                )
