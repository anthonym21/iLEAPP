...
def get_draftmessage(files_found, report_folder, seeker, wrap_text):
    data_list = []
    ...
    if data_list:
        ...
        description = ''
        report = ArtifactHtmlReport('Drafts - Native Messages')
        report.start_artifact_report(
            report_folder, 'Drafts - Native Messages', description
        )
        report.add_script()
        ...
        tsvname = 'Drafts - Native Messages'
        tsv(report_folder, data_headers, data_list, tsvname) # TODO: _csv.Error: need to escape, but no escapechar set

        tlactivity = 'Drafts - Native Messages'
        timeline(report_folder, tlactivity, data_list, data_headers)

    else:
        logfunc('No data available for Drafts - Native Messages')
